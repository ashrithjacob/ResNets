...
class Resnet(nn.Module): # to build and forward the initial convs and each of the layers
    def __init__(self, block, layer_multiple, input_channel, output_features):
        super(Resnet, self). __init__()
        
        self.block_out  = 64
        self.intermediate = 64
        self.conv1 = nn.Conv2d(input_channel, self.block_out, kernel_size=7, stride=2, padding=3)
        self.bn1 = nn.BatchNorm2d(self.block_out)
        self.relu = nn.ReLU()
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self.make_layer(block, self.block_out*1, layer_multiple[0], stride=1)
        self.layer2 = self.make_layer(block, self.block_out*2, layer_multiple[1], stride=2)
        self.layer3 = self.make_layer(block, self.block_out*4, layer_multiple[2], stride=2)
        self.layer4 = self.make_layer(block, self.block_out*8, layer_multiple[3], stride=2)
    ...
    def make_layer(self, block, out_ch, layer_multiple, stride, padding=1):
        ...
        if self.intermediate!= out_ch: 
            identity_downsample = nn.Sequential(nn.Conv2d(self.intermediate, out_ch, kernel_size=1, stride=stride, bias =False),
                                  nn.BatchNorm2d(out_ch),) 
            layers.append(block(self.intermediate,out_ch, identity_downsample, stride=stride))
